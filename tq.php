...
<title>央视最新天气预报<?php echo $aik['title']?><?php echo $aik['sitename']?></title>
    <meta name="keywords" content="央视最新天气预报，<?php echo $aik['keywords']?>" />
...
if(empty($_GET['m'])){$html='http://yys.ppdsw.xyz/tq/';
}else{ 
$html='http://yys.ppdsw.xyz/tq/'.$_GET['m'];
}
?><?php
$rurl=file_get_contents($html);
$vname='#<a href="(.*?)"><p>(.*?)</p>#';//取出链接
...
?>"  allowfullscreen="true" allowtransparency="true" style="width:100%;border:none"></iframe>');  
    ...
    var t = setTimeout(adsUp,0); 
					...
					<div class="footer clearfix">
						...
						<span class="text-muted" id="xuji">正在为您播放-天气预报<span class="js"></span></span>
					</div>
				</div>
...
//print_r($rurl);
?>' class='btn-play-source'>最新预报</a></div>
 </div>
</div>
</ul>
</div>
...
<p style="font-size:17px;"><span id=localtime></span>
<script type="text/javascript">
...
tick();
</script> </br><strong style="color: rgba(0,153,22,1.3); font-size:16px;"></strong>
</div>
</div>
			</div>
		</div>
