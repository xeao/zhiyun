...
$gaoxiao=$_GET['xiao'];
if (empty($gaoxiao)){$play=$_GET['play'];$player='https:'.$play;}
else{ $play=base64_decode($gaoxiao);$player='https:'.$play;}
?>
...
.center{color:#FFFF;} 
</style> <script src="static/js/jquery_ldg.js"></script>
...
$("#LoginShowTip").show().text("获取下载列列表失败反馈QQ:2248186422"); 
					//$(".log_box").remove();删除					
				}else{
...
function SwitchAreaa(){
    ...
    //设置延时函数
    function adsUp(){    
        $("#cms_player").html('<iframe allowFullscreen="true" src="<?php echo $aik['jiekouyy']?><?php echo $player;?>" id="video" style="width:100%;border:none" allowtransparency="true" allowfullscreen="true" frameborder="0" scrolling="no"></iframe>');  
    }
    //五秒钟后自动收起
...
<li><a class="btn btn-sm btn-default"onclick="SwitchAreaa()"> <i class="icon iconfont icon-collection"></i>视频报错</a></li>
						...
						<span class="text-muted" id="xuji">正在为您播放<?php echo $timu; ?></span>

					</div>
                        <p style="text-align:center;color: #ffff;font-size: 10px;background: #2d6296;padding:11px 8px;border-radius: 2px;"><font color="#FF0000" size='2'>小提示：若等待15秒以上还未播放，请手动切换其他线路或播放源.加载需要时间.请耐心等待!</font></p>
						...
						<div class="hy-play-list play">
							<div class="item tyui" id="dianshijuid">
								<div class="panel clearfix">
									<a class="option collapsed" data-toggle="collapse" data-parent="#playlist" href="#playlist1">播放线路<span class="text-muted pull-right"><i class="icon iconfont icon-right"></i></span></a>
									<div id="playlist1" class="playlist collapse in dianshijua">
										<ul class="playlistlink-1 list-15256 clearfix">


<li><a href="<?php echo $aik['jiekouyy']?><?php echo $player;?>" class="am-btn am-btn-default lipbtn" id="0" target="main">解析线路1</a></li><li><a href="<?php echo $aik['jiekouyy1']?><?php echo $play;?>" class="am-btn am-btn-default lipbtn" id="1" target="main">解析线路2</a></li>
<li><a href="<?php echo $aik['jiekouyy2']?><?php echo $play;?>" class="am-btn am-btn-default lipbtn" id="2" target="main">解析线路3</a></li>
	</ul>
	</div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	function jiazainame(jiid){
		...
		var tishi="";
		if (name=="") {
			tishi=('正在为您播放<?php echo $timu; ?>'+$("#"+jiid).text()+'');
		}else{
			 tishi = ('正在为您播放 <?php echo $timu; ?>'+name+'');
		}
		$("#"+jiid).attr("data-clipboard-text",'')
