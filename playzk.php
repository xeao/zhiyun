...
function SwitchAreaa(){
    ...
    //设置延时函数
    function adsUp(){    
        $("#cms_player").html('<iframe allowFullscreen="true" src="ck/?url=<?php 
...
<li><a class="btn btn-sm btn-default"onclick="SwitchAreaa()"> <i class="icon iconfont icon-collection"></i>视频报错</a></li>
						...
						<span class="text-muted" id="xuji">正在为您播放<?php echo $data['data'][0]['vod_name'];?></span>
...
<?php
                ...
                foreach($d as $k=>$v){    
                    foreach ($v as $cc){
                        ...
                        $title= $u[0];
                        echo
                        '<li><a href="'.$urls.'" class="am-btn am-btn-default lipbtn" id="0" target="main">
                        '.$title.'</a></li>';
                        }
                    }   
                }
			...
			<div class="hy-layout clearfix"><div style="margin-top:0px"></div>
				...
				<div class="tab-content">
					<div class="hy-play-list tab-pane fade in active" id="list3">
						<div class="item">
							<div class="plot">
								<font color="#FFFFFF"><font color="#FFFFFF"><?php echo $data['data'][0]['vod_content'];?>					</font></div>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	function jiazainame(jiid){
		...
		var tishi="";
		if (name=="") {
			tishi=('正在为您播放<?php echo $data['data'][0]['vod_name'];?>- '+$("#"+jiid).text()+'');
		}else{
			 tishi = ('正在为您播放<?php echo $data['data'][0]['vod_name'];?>- '+name+'');
		}
